    ...
    const checkLiveRunners = async () => {
      ...
      try {
        ...
        const validPlayers = players.filter((player) => {
          ...
          return trimmed !== "" && lower !== "null" && lower !== "undefined";
        });
          ...
          validPlayers.map(async (player) => {
            try {
              ...
              try {
                ...
                const viewerTimeoutId = setTimeout(
        ...
        const live = liveStatusChecks.filter(
          (runner) => runner !== null,
        ) as LiveRunner[];
        setLiveRunners(live);
        setHasCheckedOnce(true);
        pageCache.set("live-runners", live, 1000 * 60 * 2); // Cache for 2 minutes
      } catch {
        setLiveRunners([]);
        setHasCheckedOnce(true);
      } finally {
  ...
  return (
    <div className="min-h-screen bg-[#1e1e2e] text-ctp-text py-8 overflow-x-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 w-full">
        ...
        {isLive && !isLoading && (
          <div
            className={`grid grid-cols-1 lg:grid-cols-[1fr_280px] xl:grid-cols-[1fr_320px] gap-6 items-stretch transition-all duration-1000 delay-500 ${isLoaded ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}
          >
            {/* Stream Player */}
            <div className="w-full">
              <div
                className="bg-gradient-to-br from-[hsl(240,21%,16%)] to-[hsl(235,19%,13%)] border border-[hsl(235,13%,30%)] rounded-none overflow-hidden shadow-2xl relative"
              ...
              {/* Title below player */}
              <div
                className={`text-center mt-4 transition-all duration-1000 ${isLoaded ? "opacity-100 translate-y-0" : "opacity-0 -translate-y-4"}`}
              >
                <Badge
                  variant="default"
              ...
              {checkingRunners && !hasCheckedOnce ? (
                        ...
                        >
                          <div className="flex items-center gap-3 mb-3">
                            <Avatar className="h-10 w-10 flex-shrink-0">
                              <AvatarImage
                                src={
                                  ...
                                  `https://api.dicebear.com/7.x/lorelei-neutral/svg?seed=${runner.displayName}`
                                }
                              />
                              <AvatarFallback>
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <p className="text-xs text-ctp-overlay0 truncate">
                                @{runner.twitchUsername}
                              </p>
                            </div>
                            <Badge
                              variant="default"
                              className="bg-red-600 text-white border-0 shadow-lg flex-shrink-0"
                            >
                          ...
                          {runner.viewerCount !== undefined && (
                            <div className="mt-3 flex items-center justify-center">
                              <Badge
                                variant="outline"
                                className="text-xs border-[hsl(235,13%,30%)] bg-[hsl(235,13%,15%)]"
                              >
                                <span className="font-semibold">
                                  {runner.viewerCount.toLocaleString()}
                                </span>
                                <span className="ml-1 text-ctp-subtext1">
                                  {runner.viewerCount === 1
                                    ? t("live.viewer")
                                    : t("live.viewers")}
                                </span>
                              </Badge>
                ...
                <div className="text-center py-12">
                  <div className="flex flex-col items-center gap-3">
                    ...
                    <div>
                      <h3 className="text-base font-semibold mb-1 text-ctp-text">
                        {t("live.noRunnersLive")}
                      </h3>
