...
> {
  ...
  const { ourCategoryId, categoryName } = extractCategory(
    run,
    categoryMapping,
    srcCategoryIdToName,
    categoryNameMapping,
  ...
  const { ourPlatformId, platformName } = await extractPlatform(
    run,
    platformMapping,
    srcPlatformIdToName,
    platformNameMapping,
  ...
  const { ourLevelId, levelName } = extractLevel(
    run,
    levelMapping,
    srcLevelIdToName,
  );
  ...
  const { subcategoryId, srcSubcategory } = extractSubcategory(
    run,
    leaderboardType,
    ourCategoryId,
    localCategories,
